...
For details and the rationales ot each requirement please have a look the corresponding subsections below.
...
This implies, that all frontend applications must trusted (e.g. the \emph{Clotilde UI} and all integrated \emph{Monitor} components).
...
\subsubsection{S2. The users receiving a shared log can verify that the data owner has intended to share the log with them.}
...
\subsubsection{S3. The users receiving a shared log can verify that the log was created by a valid monitor.}
