...
The main purpose of the protocol is the secure encryption of logs in the toolchain while adding the possibility to share them.
...
Finally, section~\ref{sec:jose-protocol} illustrates how the JOSE standard can be used to practically implement the protocol.
...
This arises the question, whether encrypted data should be signed (\emph{encrypt-then-sign}) or whether signed data should be encrypted (\emph{sign-then-encrypt}).
...
\begin{enumerate}
    \item 
    An encrypted log is singed by the creator along with the set of recipients.
...
This allows the server to filter logs concerning a particular user.
...
Since JWS and JWE tokens can also be nested into each other, JOSE cab be used to realize the desired protocol by the following means:
\begin{enumerate}
    \item 
...
The monitor must not include its own identity in the set of authorized users.
...
It allows the verification if the data owner has intended to share the log with respective receiver.
