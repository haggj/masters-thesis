...
The approach of hybrid encryption is the extension of the ideas introduced in the previous section \ref{sec:mutual-encryption}.
The idea of hybrid encryption combines the advantages of asymmetric cryptography with the advantages of symmetric cryptography.
In a hybrid scheme, a symmetric key is encrypted with the asymmetric algorithms.
...
She then encrypts $k$ under Bobs' public key.
...
A initially created log is encrypted for Alice using hybrid encryption.
This results in a cipher which consist of a encrypted symmetric key and of the encrypted log.
...
She generates a fresh symmetric key and encrypts it under Bobs' public key.
...
Alice can also revoke the access to the log by re-encrypting the log. 
This requires a fresh symmetric key $k$.

\begin{figure}[ht]
    \includegraphics[scale=0.2]{../img/04/hybrid_encryption.jpg}
    \centering
    \caption{Structure of a encrypted log using hybrid encryption.}
...
Moreover, only users can decrypt the cipher which were explicitly specified during encryption.
